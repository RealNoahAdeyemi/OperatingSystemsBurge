...
void allocate_memory(list_t * freelist, list_t * alloclist, int pid, int blocksize, int policy) {
    // Find index of chosen free block according to policy
    int chosen_index = -1;
    ...
    if (policy == 1) { // FIFO (first fit)
        while (cur) {
            int sz = (cur->blk->end - cur->blk->start) + 1;
            if (sz >= blocksize) {
                chosen_index = idx;
                break;
            }
    ...
    } else if (policy == 2) { // BEST FIT - smallest block >= blocksize
    ...
    } else { // WORST FIT - largest block >= blocksize
        int worst_size = -1;
        idx = 0;
        cur = freelist->head;
        int worst_index = -1;
        ...
        while (cur) {
            int sz = (cur->blk->end - cur->blk->start) + 1;
            if (sz >= blocksize && sz > worst_size) {
                worst_size = sz;
                worst_index = i;
            }
            cur = cur->next; i++;
        }
        chosen_index = worst_index;
    }
    ...
    block_t *blk = list_remove_at_index(freelist, chosen_index);
    if (!blk) {
        printf("Error: Memory Allocation %d blocks\n", blocksize);
        return;
    }

    int original_start = blk->start;
    ...
    // Add to allocated list in ascending address order
    list_add_ascending_by_address(alloclist, blk);

    // If there is remaining fragment, create and add to freelist according to policy
...
void deallocate_memory(list_t * alloclist, list_t * freelist, int pid, int policy) { 
    // find the index of pid in alloclist
    ...
    // add back to free list according to policy
    if (policy == 1) {
